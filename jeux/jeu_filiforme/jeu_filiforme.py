...
class LutinPorte(Lutin):
    ...
    def ouvrir_porte(self):
        self.jeu.canvas.itemconfig(self.image, image=self.porte_ouverte)
        self.jeu.tk.update_idletasks()

    def fermer_porte(self):
        self.jeu.canvas.itemconfig(self.image, image=self.porte_fermee)
        self.jeu.tk.update_idletasks()
...
class LutinPersonnage(Lutin):
    ...
    def __init__(self, jeu):
        ...
        jeu.canvas.bind_all('<KeyPress-Up>' , self.stabiliser)
