...
"""
le jeu de rebonds de plusieurs balles sur une raquette 
"""
...
class Balle:
    ...
    def dessiner(self,balles):
        ...
        if self.heurter_raquette(self.raquette1,pos) == True:
            self.y = -3
				# créer les balles supplémentaires et fantômes
            balles.append(Balle(canvas, raquette1, 'blue'))
...
balles = []
balles.append(Balle(canvas, raquette1, 'red'))
...
while(1):
    if balles[0].touche_bas == False:
        if niveau > 1 or (niveau == 1 and passage % 2 == 0):
            for balle in balles:
                balle.dessiner(balles)
        raquette1.dessiner()
        tk.update_idletasks()
        tk.update()
        time.sleep(0.01)
