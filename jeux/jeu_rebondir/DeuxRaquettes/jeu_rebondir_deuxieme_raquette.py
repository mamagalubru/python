...
# TEST POUR LA DEUXIEME RAQUETTE

"""
le jeu de rebonds de balle sur 2 raquettes
"""
...
class Balle:
    ...
    def heurter_raquette2(self, raquette2, pos):
        global coups
        # récupération des 4 coordonnées de la raquette (haut-gauche, bas-droit)
        pos_raquette2 = self.canvas.coords(raquette2.id)
        # comparaison avec les 4 coordonnées de la balle
        if pos[2] >= pos_raquette2[0] and pos[0] <= pos_raquette2[2]:
            if pos[3] >= pos_raquette2[1] and pos[3] <= pos_raquette2[3]:
                # si oui il y a collision
                coups = coups + 1
                return True
            return False
...
class Raquette:
    ...
    def __init__(self, canvas, couleur):
        ...
        self.canvas.bind_all('<KeyPress-s>',self.stationnaire)
...
# créer un canevas
canvas = Canvas(tk, width=500, height=700, bd = 0, highlightthickness=0)
canvas.pack()
tk.update()

# créer une (ou plusieurs) raquette
raquette1 = Raquette(canvas, 'black')
raquette2 = Raquette2(canvas, 'red')

# créer une (ou plusieurs) balle
balle = Balle(canvas, raquette1, raquette2, 'white')
#balle2 = Balle(canvas, raquette2, 'white')
...
while(1):
    if balle.touche_bas == False:
        if niveau > 1 or (niveau == 1 and passage % 2 == 0):
            ...
            #balle2.dessiner()
        raquette1.dessiner()
        #Rajoute d'une deuxième raquette
        raquette2.dessiner()
        #----------
        tk.update_idletasks()
        tk.update()
