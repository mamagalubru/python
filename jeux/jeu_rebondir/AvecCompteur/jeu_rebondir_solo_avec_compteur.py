...
class Balle:
    ...
    def heurter_raquette(self, raquette, pos):
        ...
        if pos[2] >= pos_raquette[0] and pos[0] <= pos_raquette[2]:
            if pos[3] >= pos_raquette[1] and pos[3] <= pos_raquette[3]:
                ...
                coups = coups + 1
                # créer le compteur
                contour_compteur = canvas.create_rectangle(10, 400, 100, 320, fill="grey")
                compteur = canvas.create_text(90,370, text='SCORE\n' + str(coups), font=('Helvetica',14), fill="#2d2d2d", anchor=E)
                return True
            return False
    ...
    def dessiner(self):
        ...
        if pos[1] <= 0:
            ...
            winsound.PlaySound('C:/Windows/Media/ir_fin.wav',winsound.SND_FILENAME|winsound.SND_ASYNC)
