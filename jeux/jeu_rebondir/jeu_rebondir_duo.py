...
"""
le jeu de rebonds de balle sur 2 raquettes poussé le 21 mars à 22h
"""
...
class Balle:
    ...
    def __init__(self, canvas, raquette1, raquette2, couleur):
        self.canvas = canvas
    ...
    def dessiner(self):
        ...
        # teste le contact avec la raquette 2 (si oui remonte)
        if self.heurter_raquette(self.raquette2,pos) == True:
            self.y = -3
            # jouer une musique de raquette sous windows
            winsound.PlaySound('C:\\Python34\\tests\\sons\\pong_raquette.wav',winsound.SND_FILENAME|winsound.SND_ASYNC)
...
class Raquette:
    ...
    def __init__(self, canvas, couleur):
        ...
        self.largeur_canevas = self.canvas.winfo_width()
        # positionner la raquette dans le canvas et lier les touches de contrôles en fonction de l'id raquette
        # les 2 id étant successifs on est certain que l'un sera pair et l'autre impair d'ou différence par modulo
        if self.id % 2 == 1:
            self.canvas.move(self.id, 100, 300)
            self.canvas.bind_all('<KeyPress-i>',self.vers_gauche)
            self.canvas.bind_all('<KeyPress-p>',self.vers_droite)
            self.canvas.bind_all('<KeyPress-o>',self.stationnaire)
        else:
            self.canvas.move(self.id, 400, 300)
            self.canvas.bind_all('<KeyPress-a>',self.vers_gauche)
            self.canvas.bind_all('<KeyPress-e>',self.vers_droite)
            self.canvas.bind_all('<KeyPress-z>',self.stationnaire)
...
raquette2 = Raquette(canvas, 'yellow')

# créer une (ou plusieurs) balle
balle = Balle(canvas, raquette1, raquette2, 'red')
